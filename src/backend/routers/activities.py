...
def get_activities(
    ...
    - difficulty_level: Filter activities by difficulty level ('Beginner', 'Intermediate', 'Advanced', 'All')
    """
    ...
    if difficulty_level:
        if difficulty_level == "All":
            # For "All", only include activities without difficulty_level
            query["difficulty_level"] = {"$exists": False}
        else:
            # For specific difficulty levels, only include activities with that level
